...
type Props = {
    ...
    setValue: React.Dispatch<React.SetStateAction<{ reassignmentReason?: string; businessId?: string, tid?: string }>>
    handleSubmit?: (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => void
}

const ReassignTerminalWidget = ({ data, loadingOptions, triggerSubmit, isShown, value, handleSubmit, toggleModal, setValue, loading, merchants }: Props) => {
console.log(loading, loadingOptions, "opk")
    const mappedMerchants = mapMerchants(merchants);
    console.log(mappedMerchants, "merchants")
    console.log(data)
    const subtitle = data?.user_id === null
        ? 'Assign this terminal to a merchant'
        : 'Reassign this terminal to a new merchant'
    const btnText = data?.user_id === null
        ? 'Assign Terminal'
        : 'Reassign Terminal'
    return (
        <Modal
            ...
            {data?.user_id !== null && (
                <>
                    ...
                    <TextArea
                        ...
                        textAreaWidth="95%"
                        onChange={(e) => setValue({ ...value, reassignmentReason: e.target.value })}
                    ></TextArea>
                </>)}
            {triggerSubmit && data?.user_id === null && value.reassignmentReason === ''
                && <Text
                    padding="8px"
                    as={'small'}
                    weight={'500'}
