...
import { ButtonWrapper, TerminalDetailWrapper } from "./styles/tab-content.styles";
...
const TerminalDetails = (data: any) => {
    ...
    const buttonEnabledText = data?.data?.data?.active ? "Disable Terminal" : "Enable Terminal";
    const enabledSubTitle = data?.data?.data?.active ? "Disable this terminal" : "Enable this terminal";
    const enabledPlaceholder = data?.data?.data?.active ? "Enter reason for disabling terminal" : "Enter reason for enabling terminal";
    return (
        <TerminalDetailWrapper>
            <Modal
                subTitle={assigned ? "Reassign this terminal to a new merchant" : enabledSubTitle}
            ...
            <ButtonWrapper>
                <Button height="3.2rem" radius="10px" borderSize="1px" width="14%" onClick={toggleEnable} color={Color.alerzoBlueTint} borderColor={Color.alerzoBlueTint} variant="transparent">
                   {buttonEnabledText}
                </Button>
                <Button height="3.2rem" width="14%" radius="10px" onClick={toggleAssigned}>
                    Reassign Terminal
                </Button>
            </ButtonWrapper>
        </TerminalDetailWrapper>
