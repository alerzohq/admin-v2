            ...
            <>
                        ...
                        {!specsLoading && (
                            <SelectInput
                                ...
                                options={[
                                    ...
                                    ...specs?.data?.map(
                                        (spec: { variant: string; id: string, serial_number:string, }) => {
                                            return {
                                                value: spec.id,
                ...
                {mutation.isError && (
                    <Text
                        ...
                        color={Color.alerzoDanger}
                    >
                        {mutation.error.response.data?.stack ===  `Error: Terminal has already been assigned\n    at AssignTerminal.execute (/usr/app/.dist/src/services/admin/terminals/assign-terminal.js:44:19)\n    at async Object.AdminController.assignTerminal (/usr/app/.dist/src/api/http/controllers/admin.js:272:28)`  ? "Terminal has already being assigned": mutation.error.response.data.message as string}
                    </Text>
                )}
            </>
        </Modal>
